...
POLICY_DOCUMENT='deltaProtect-custom-policy.json'
...
JSON_FILE=deltaProtect-custom-policy.json
...
POLICY_FILE='deltaProtect-customPolicy.json'

POLICY_NAME='deltaProtect-customPolicy'
...
ROLE_ARN=$(aws iam list-roles | grep $ROLE_NAME | grep Arn | sed 's/"//g' | sed 's/,//' | awk '{print $2}')
